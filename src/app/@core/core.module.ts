...
...NbAuthModule.forRoot({
    strategies: [
      NbPasswordAuthStrategy.setup({
        name: 'email',
        baseEndpoint: environment.url, // Your Django URL
        
        login: {
          endpoint: '/token/', // Django SimpleJWT login endpoint
          method: 'post',
          redirect: {
            success: '/pages/formatter', // Where to go after login
            failure: null,
          },
        },
        register: {
            endpoint: '/register/',
          method: 'post',
          requireValidToken: true, // <--- NOW TRUE: We expect a token!
          redirect: {
            success: '/pages/formatter', // <--- Go straight to the app (Auto-Login)
            failure: null,
          },
    },
    requestPass: {
    ...
    redirect: {
      ...
      failure: null,
        ...
        token: {
          ...
          key: 'access', // <--- CRITICAL: Django SimpleJWT calls the token 'access'
        },
      }),
    ],
    forms: {
